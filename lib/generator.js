...
function getContainerHostData(labelPrefix, container, callback) {
  container.inspect(function(error, data) {
    ...
    if (!('Config' in data) || data.Config === null) return callback(new Error(`Missing section Config in inspect data of container ${container.id.substr(0, 12)}`));
    if (!('Labels' in data.Config) || data.Config.Labels === null) return callback();
    ...
    if (!('NetworkSettings' in data) || data.NetworkSettings === null) return callback(new Error(`Missing section NetworkSettings in inspect data of container ${container.id.substr(0, 12)}`));

    let defaultPort = null;
    for (let host in virtualHostsAndPorts) {
      if (virtualHostsAndPorts[host]) continue;
      if (!defaultPort) {
        if (!('Ports' in data.NetworkSettings) || data.NetworkSettings.Ports === null) return callback(new Error(`Missing property Ports in NetworkSettings of container ${container.id.substr(0, 12)}`));
    ...
    if (!('IPAddress' in data.NetworkSettings) || data.NetworkSettings.IPAddress === null) return callback(new Error(`Missing property IPAddress in NetworkSettings of container ${container.id.substr(0, 12)}`));

    let virtualAddress = data.NetworkSettings.IPAddress;
    if (virtualAddress === '') return callback();
