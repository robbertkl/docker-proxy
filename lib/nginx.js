...
class Nginx {
  constructor(options) {
    ...
    privateData.set(this, {
      ...
      configFilePath: options.configFilePath || `/etc/nginx/nginx-${crypto.pseudoRandomBytes(6).toString('hex')}`,
      config: null,
      process: null,
      ...
      execFile(data.executablePath, ['-c', configFilePathTest, '-q', '-t'], function(error, stdout, stderr) {
        if (error) {
          ...
          for (let line of lines) {
            let matches = line.match(/^nginx: \[.*\] (.*) in .*:(\d+)$/);
            if (!matches) continue;
            filtered.push(`${matches[1]} on line ${matches[2]}`);
          }
